...
class ApiExceptionHandler {
    ...
    @ExceptionHandler(ValidationIssuesException::class)
    fun validationIssues(invalid: ValidationIssuesException): ResponseEntity<Any> =
        Api.respond(
            HttpStatus.BAD_REQUEST,
            mapOf(
                "success" to false,
                "message" to "Validation failed",
                "issues" to invalid.issues.map { mapOf("code" to it.code, "message" to it.message) },
            ),
        )
